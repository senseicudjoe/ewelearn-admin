  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[280px] gap-4">
        ...
        <p className="text-sm font-medium text-slate-500">Loading pending lessons...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border-b border-slate-200/80 pb-6">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Pending Review</h1>
      ...
      {pendingLessons.length === 0 ? (
        <Card className="border-slate-200/80 shadow-sm">
          <CardContent className="py-14 text-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50">
            ...
            <p className="text-slate-500">No lessons waiting for review</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {pendingLessons.map((lesson) => {
            ...
            return (
              <Card key={lesson.lessonId} className="border-slate-200/80 shadow-sm transition-all hover:shadow-md hover:border-slate-300/80">
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        <CardTitle className="text-lg font-semibold text-slate-900">{lesson.title}</CardTitle>
                        <Badge variant="secondary" className="text-amber-700 bg-amber-100 border-amber-200">
                          <Clock className="w-3 h-3 mr-1" />
                          Pending
                        </Badge>
                      </div>
                      <CardDescription className="mt-2 space-y-1 text-slate-500">
                        ...
                        {module && (
                          <div className="text-sm">Module: <strong className="text-slate-700">{module.title}</strong></div>
                        )}
                      </CardDescription>
                    </div>
                    <Button onClick={() => openReviewDialog(lesson)} className="bg-purple-600 hover:bg-purple-700 shrink-0">
                      Review
                    </Button>
                  </div>
                </CardHeader>
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm font-medium text-slate-700 mb-1">Content Preview</p>
                      <p className="text-sm text-slate-600 line-clamp-3 rounded-lg bg-slate-50/80 p-3">
                        {lesson.content}
                      </p>
                    </div>
